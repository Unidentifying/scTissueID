#############################
#NMC
#Run: python3 NMC example/trains.csv example/trainslabel.csv example/tests.csv example/testslabel.csv ./
#Input args:
#	Input: 
...
tstart		= tm.time()
NMC.fit(train, y_train)
traintime	= tm.time()-tstart

tstart		= tm.time()
predicted	= NMC.predict(test)
...
truelab.to_csv(str(Output/Path("NMCtruelabel.csv")), index = False)
pred.to_csv(str(Output/Path("NMCpredlabel.csv")), index = False)    
